...
export default function MicrophoneComponent({ callUnityFunction }) {
  ...
  const { setFaultyPart, faultyPart, setFunctionName, setCommandName, setCommandMicClicked, commandMicClicked, setIsAssembled, setShowVoiceCommands } = useStateStore();
  ...
  useEffect(() => {
    ...
    if (transcript.includes("disassemble")) {
      ...
      CallUnityFunction("SetHome")
    }
    else if (transcript.includes("open")) {
      CallUnityFunction("ShowModule")
    }
    ...
    else if (transcript.includes("error 400")) {
      CallErrorFunction("LiftingMechanism")
    }
    else if (transcript.includes("error 300")) {
      CallErrorFunction("Shuttle")
    }
    else if (transcript.includes("error 200")) {
      CallErrorFunction("Storage")
    }
    else if (transcript.includes("error 100")) {
